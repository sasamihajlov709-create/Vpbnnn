...
enum class ObservationQuality(val weight: Double, val minLevelForHostMemory: Boolean, val label: String) {
    CONNECT_ONLY(0.0, false, "Probing"),               // TCP SYN-ACK only - does not prove bypass
    TLS_RECORD_RECEIVED(0.1, false, "Viable"),        // Initial TLS/HTTP response chunk received (weak bypass signal)
    SERVER_HELLO_RECEIVED(0.5, false, "Promising"),      // Handshake in progress, got Server Hello
    HANDSHAKE_COMPLETE(1.0, false, "Likely Good"),          // Cryptographic handshake / full TLS session established
    APPLICATION_DATA_EXCHANGED(2.0, true, "Optimal"),  // Valid application payload transferred without RST/stall
