...
object BypassConfig {
    ...
    fun getSessionConfig(host: String, strategy: BypassStrategy, rtt: Long): SessionConfig {
        ...
        // Dynamic MSS calculation to evade DPI fingerprinting
        val mss = when {
            intensity > 90 -> rnd.nextInt(128, 512)
            intensity > 75 -> rnd.nextInt(512, 1000)
            intensity > 50 -> 1100 + rnd.nextInt(100)
            else -> 1440
        }
    ...
    fun startLearningTask(scope: CoroutineScope) {
        ...
        learningJob = scope.launch(ProxyDispatcher.io) {
            ...
            while (isActive) {
                ...
                // 1. Proactive MTU tuning if success rate is dropping
                if (ProxyStats.successRate.value < 75 && ProxyStats.activeConnections.value > 0) {
                     val currentMtu = _currentMtu.value
                     if (currentMtu > 1200) {
                         setMtu(currentMtu - 50)
                         ProxyStats.logRecovery("Proactive Optimization: Reducing MTU to ${_currentMtu.value} due to low success rate.")
                     }
                }
                ...
                if (ProxyStats.activeConnections.value == 0) {
                    ...
                    if (java.util.concurrent.ThreadLocalRandom.current().nextInt(100) < 40) {
                        val bestMtu = AutoTtlProber.probeBestMtu(target, 443, activeVpnService)
                        if (bestMtu in 576..1499 && bestMtu < _currentMtu.value) {
                             setMtu(bestMtu)
                        }
                    }
                }
                
                // Cleanup host memories periodically
                val now = System.currentTimeMillis()
                val iterator = hostStrategyMemory.iterator()
                while (iterator.hasNext()) {
                    val entry = iterator.next()
                    if (now > entry.value.second) {
                        iterator.remove()
                    }
                }
            }
    ...
    private val bypassList = hashSetOf(
        ...
        "tele2.ru", "rt.ru", "tinkoff.ru", "vtb.ru", "alfabank.ru",
    ...
    fun isHostDirect(host: String): Boolean {
        ...
        if (h.startsWith("172.") && h.length >= 7) {
            val secondOctet = try {
                val dotIdx = h.indexOf('.', 4)
                if (dotIdx != -1) h.substring(4, dotIdx).toInt() else 0
            } catch (e: Exception) { 0 }
            if (secondOctet in 16..31) return true
        }

        // Fast check using pre-defined list
        ...
        when (config.strategy) {
            ...
            BypassStrategy.UDP_SKEW_ADVANCED -> {
                try {
                    ...
                    // 1. Send packet with fake data at the end + low TTL
                    ...
                    // 2. Real packet
                    TtlHelper.setUdpTtl(socket, oldTtl, isIpv6)
                    socket.send(packet)
            ...
            BypassStrategy.UDP_COMBINED_NUCLEAR -> {
                val isIpv6 = targetAddr is java.net.Inet6Address
                // 1. Double Shadow: Send two different fake protocol handshakes with very low TTL
                try {
                    TtlHelper.setUdpTtl(socket, rnd.nextInt(2, 4), isIpv6)
                    val fake1 = FakePacketHelper.buildProtocolConfusion("DTLS")
                    socket.send(DatagramPacket(fake1, fake1.size, targetAddr, targetPort))
                    delay(1)
                    val fake2 = FakePacketHelper.buildProtocolConfusion("QUIC")
                    socket.send(DatagramPacket(fake2, fake2.size, targetAddr, targetPort))
                    delay(rnd.nextLong(2, 5))
                } catch (e: Throwable) {}

                // 2. Fragmented and Reordered real data with Redundancy
                TtlHelper.setUdpTtl(socket, 64, isIpv6)
                val split = (length / 2).coerceIn(1, length - 1)
                
                // Redundancy: send a copy of the whole packet first (might be blocked, but good if first part is lost)
                if (rnd.nextBoolean()) {
                    socket.send(packet)
                    delay(1)
                }
            ...
            BypassStrategy.UDP_COMBINED_NUCLEAR -> {
                ...
                // 1. Initial Handshake Camouflage (QUIC/DTLS)
                if (ProxyStats.censorshipIntensity.value > 60) {
                    val fake = if (rnd.nextBoolean()) FakePacketHelper.buildQuicCryptoFake() else FakePacketHelper.buildUdpNoise(rnd.nextInt(32, 128))
                ...
                // 2. Nuclear Fragment/Stutter
                val chunkSize = rnd.nextInt(64, 512)
                var pos = offset
                while (pos < offset + length) {
                    val remaining = (offset + length) - pos
                    val sz = chunkSize.coerceAtMost(remaining)
                    
                    // Injection of junk overlap
                    if (remaining > 100 && rnd.nextInt(100) < 30) {
                    ...
                    pos += sz
                    // Stutter delay
                    if (pos < offset + length) delay(rnd.nextLong(2, 12))
                }
            }
            BypassStrategy.UDP_COMBINED_HYBRID -> {
                val isIpv6 = targetAddr is java.net.Inet6Address
                // 1. Send Chaos/Noise Packet with low TTL
                ...
                // 2. Fragment and Reorder with Skew
                TtlHelper.setUdpTtl(socket, 64, isIpv6)
                val split = (length / 2).coerceIn(1, length - 1)
                if (rnd.nextBoolean()) {
                    socket.send(DatagramPacket(data, offset + split, length - split, targetAddr, targetPort))
            ...
            BypassStrategy.UDP_PADDING_CHAOS -> {
                ...
                if (rnd.nextBoolean()) {
                     TtlHelper.setUdpTtl(socket, rnd.nextInt(3, 6), targetAddr is java.net.Inet6Address)
    ...
    suspend fun applyBypass(socket: Socket, output: OutputStream, data: ByteArray, length: Int, config: SessionConfig, host: String) {
        ...
        when (strategy) {
            ...
            BypassStrategy.TCP_ZERO_WINDOW_OOB -> {
                try {
                    val originalSize = socket.receiveBufferSize
                    // 1. Drop window to 0 to stop DPI from seeing the next segments
                    socket.receiveBufferSize = 0
                    delay(rnd.nextLong(20, 100))
                    
                    // 2. Send OOB "junk" that DPI might try to parse as the start of stream
                    try { socket.sendUrgentData(0xFF) } catch(e: Throwable) {}
                    
                    // 3. Send first half of data with tiny window
                    ...
                    // 4. Restore window and send rest
            ...
            BypassStrategy.TCP_TLS_HELLO_FRAGMENT -> {
                try {
                    val isHello = TlsParser.isClientHello(finalData, finalLen)
                    if (isHello) {
                        // Split Client Hello into tiny fragments
                        var i = 0
                        while (i < finalLen) {
                            val chunk = if (i == 0) rnd.nextInt(1, 5) else rnd.nextInt(2, 20)
                            val toWrite = (finalLen - i).coerceAtMost(chunk)
                            output.write(finalData, i, toWrite)
                            output.flush()
                            i += toWrite
            ...
            BypassStrategy.TCP_TLS_SNI_CASE_MOD -> {
                try {
                    val sniOffset = TlsParser.findSniOffset(finalData, finalLen)
                    if (sniOffset != -1) {
                        val nameLen = ((finalData[sniOffset - 2].toInt() and 0xFF) shl 8) or (finalData[sniOffset - 1].toInt() and 0xFF)
                        val modified = finalData.copyOf()
                        ...
                        output.write(modified, 0, finalLen); output.flush()
                    } else {
                        output.write(finalData, 0, finalLen); output.flush()
                    }
                } catch (e: Throwable) { output.write(finalData, 0, finalLen); output.flush() }
            ...
            BypassStrategy.TCP_REORDER -> {
                if (finalLen < 100 && rnd.nextInt(100) < 30) {
            ...
            BypassStrategy.TCP_WINDOW_SIZE_OSCILLATION -> {
                try {
                    val originalSize = socket.receiveBufferSize
                    val chunks = rnd.nextInt(3, 6)
                    var pos = 0
                    for (i in 0 until chunks) {
                        val remaining = finalLen - pos
                        if (remaining <= 0) break
                        val sz = (remaining / (chunks - i)).coerceAtLeast(1)
                        socket.receiveBufferSize = if (i % 2 == 0) rnd.nextInt(128, 512) else rnd.nextInt(1024, 8192)
            ...
            BypassStrategy.TCP_TLS_SESSION_DESYNC -> {
                try {
                    ...
                    if (offset != -1) {
                        // Send fake Session ID extension to desync DPI session tracking
                        val fakeSessionId = ByteArray(32) { rnd.nextInt(256).toByte() }
                        val mod = FakePacketHelper.injectExtension(finalData, finalLen, 0x0023, fakeSessionId) // Ticket extension
                        output.write(mod, 0, offset + 2); output.flush(); delay(rnd.nextLong(5, 15))
                        output.write(mod, offset + 2, mod.size - (offset + 2)); output.flush()
                    } else {
                        output.write(finalData, 0, finalLen); output.flush()
                    }
                } catch (e: Throwable) { output.write(finalData, 0, finalLen); output.flush() }
            ...
            BypassStrategy.TLS_SNI_FRAGMENT -> {
                ...
                if (offset != -1 && host.length > 2) {
                    ...
                    output.write(finalData, 0, split1); output.flush(); delay(rnd.nextLong(1, 5))
                    output.write(finalData, split1, split2 - split1); output.flush(); delay(rnd.nextLong(1, 5))
                    output.write(finalData, split2, finalLen - split2); output.flush()
                } else {
                    output.write(finalData, 0, finalLen / 2); output.flush(); delay(2)
            ...
            BypassStrategy.HTTP_CHUNKED_FAKE -> {
                if (isProbableHttp(finalData, finalLen)) {
                    try {
                        // We don't do full chunked encoding (hard to do without buffering whole request)
                        // but we send the request in tiny fragments with random small delays
                        val chunkSize = rnd.nextInt(1, 16)
                        var p = 0
                        while (p < finalLen) {
                            val nextLen = (finalLen - p).coerceAtMost(chunkSize)
                            output.write(finalData, p, nextLen); output.flush()
                            p += nextLen
                            if (rnd.nextBoolean()) delay(1)
                        }
                    } catch (e: Throwable) { output.write(finalData, 0, finalLen); output.flush() }
            ...
            BypassStrategy.TLS_MULTI_SNI -> {
                try {
                    val mod = FakePacketHelper.injectMultipleSni(finalData, finalLen, host)
            ...
            BypassStrategy.PROTOCOL_CONFUSION_REDIS -> {
                ...
                writeWithFake(socket, output, fake, finalData, finalLen, config)
            }
            ...
            BypassStrategy.ECH_FRAG -> {
                // ECH detection implies we need extreme fragmentation for the first part of ClientHello
                // to hide the ECH extension and Inner ClientHello
                val rnd = java.util.concurrent.ThreadLocalRandom.current()
                ...
                // Inject fake segment between fragments
                if (ProxyStats.censorshipIntensity.value > 60) {
...
TtlHelper.setTtl(socket, 64)
            ...
            BypassStrategy.TLS_REHANDSHAKE_FAKE, BypassStrategy.TCP_FAST_RETRANSMIT_SIM, BypassStrategy.TLS_LEGACY_HELLOS, BypassStrategy.TLS_SESSION_TICKET_SKEW, BypassStrategy.TLS_0RTT_FAKE, BypassStrategy.TLS_COMPRESSION_FAKE, BypassStrategy.HTTP_PIPELINE_FAKE -> {
            }
            ...
            BypassStrategy.TCP_SEGMENT_DESYNC -> {
                try {
                    ...
                    // 2. Part 2 FAKE with low TTL (DPI thinks it's the end of request)
                    TtlHelper.setTtl(socket, discoveredTtl)
                    output.write(FakePacketHelper.buildUdpNoise(length - split))
                    output.flush()
                    
                    // 3. Part 2 REAL with normal TTL
                    delay(rnd.nextLong(10, 30))
                    TtlHelper.setTtl(socket, oldTtl)
                    output.write(data, split, length - split); output.flush()
                } catch(e: Throwable) { output.write(data, 0, length); output.flush() }
            ...
            BypassStrategy.TCP_TIMING_CHAOS -> {
                try {
                    for (i in 0 until length) {
                        ...
                        output.flush()
                        if (rnd.nextBoolean()) delay(rnd.nextLong(1, 10))
                    }
                } catch (e: Throwable) { output.write(data, 0, length); output.flush() }
            }
            ...
            BypassStrategy.TCP_FOOL_DPI -> {
                try {
                    // Complex combo: Decoy Protocol + Tiny Window + OOB + Fragment
                    val decoy = FakePacketHelper.buildHandshakeCombo(rnd.nextInt(32, 64))
                    val oldTtl = TtlHelper.getSocketTtl(socket)
                    val oldWindow = socket.receiveBufferSize
                    
                    // 1. Send Decoy with low TTL
                    TtlHelper.setTtl(socket, rnd.nextInt(2, 4))
                    output.write(decoy); output.flush()
                    delay(rnd.nextLong(2, 10))
                    
                    // 2. Shrink window and send first byte
                    socket.receiveBufferSize = 1
                    TtlHelper.setTtl(socket, oldTtl)
                    output.write(data[0].toInt()); output.flush()
                    try { socket.sendUrgentData(rnd.nextInt(256)) } catch(e: Throwable) {}
                    
                    // 3. Restore and send rest
                    delay(rnd.nextLong(5, 20))
                    socket.receiveBufferSize = oldWindow
                    if (length > 1) {
                        output.write(data, 1, length - 1); output.flush()
                    }
                } catch (e: Throwable) { output.write(data, 0, length); output.flush() }
            ...
            BypassStrategy.TCP_DATA_DESYNC_OVERLAP -> {
                try {
                    ...
                    // 2. Overlap: Send REAL Part 2 with low TTL first (DPI sees this)
                    TtlHelper.setTtl(socket, discoveredTtl)
                    output.write(data, split, length - split); output.flush()
                    
                    // 3. Send FAKE noise with low TTL (DPI sees this too)
                    output.write(FakePacketHelper.buildUdpNoise(rnd.nextInt(20, 64)))
                    
                    // 4. Send REAL Part 2 with normal TTL
                    delay(rnd.nextLong(20, 50))
                    TtlHelper.setTtl(socket, oldTtl)
                    output.write(data, split, length - split); output.flush()
                } catch (e: Throwable) { output.write(data, 0, length); output.flush() }
            ...
            BypassStrategy.BYEBYEDPI_EXTREME -> {
                try {
                    ...
                    // Shake MSS and Window
                    TtlHelper.setMss(socket, rnd.nextInt(200, 500))
                    TtlHelper.setWindowSize(socket, rnd.nextInt(256, 1024))
                    
                    // Chaos Bombing (Low TTL)
                    if (rnd.nextInt(100) < 50) {
                        TtlHelper.setLowTtlTemporary(socket, rnd.nextInt(2, 4), 0)
                        val chaos = if (rnd.nextBoolean()) FakePacketHelper.buildDnsChaosPacket() else FakePacketHelper.buildTlsChaosPacket()
                        output.write(chaos); output.flush()
                        TtlHelper.setLowTtlTemporary(socket, 64, 1)
                    }
            ...
            BypassStrategy.TCP_REORDER -> {
                val split = (length / 2).coerceIn(1, length - 1)
                // Send second part first to confuse DPI reassembly
                // Note: Kernel still sequences them, so we need to use small delays
                output.write(data, split, length - split); output.flush()
                delay(rnd.nextLong(2, 5))
                output.write(data, 0, split); output.flush()
            }
            ...
            BypassStrategy.TCP_TLS_SNI_CASE_MOD -> {
                ...
                if (offset != -1) {
                    val mod = data.copyOf(length)
                    // Mangle SNI case randomly
                    val end: Int = offset + (length - offset).coerceAtMost(32)
                    for (i in offset until end) {
                        if (mod[i] in 'a'.code.toByte()..'z'.code.toByte() && rnd.nextBoolean()) {
                            mod[i] = (mod[i] - 32).toByte()
                        }
                    }
                    output.write(mod); output.flush()
                } else {
            ...
            BypassStrategy.TCP_WINDOW_STALL -> {
                try {
                    // Start with 0 window to stall DPI, then open
                    try { TtlHelper.setWindowSize(socket, 0) } catch(e: Throwable) {}
                    delay(config.delay1.coerceAtLeast(100L))
            ...
            BypassStrategy.TCP_TLS_HELLO_FRAGMENT -> {
                if (length > 10 && data[0] == 0x16.toByte()) {
                    // Fragment exactly at the version field
                    output.write(data, 0, 3); output.flush()
                    delay(rnd.nextLong(1, 3))
                    output.write(data, 3, length - 3); output.flush()
                } else {
                    output.write(data, 0, length); output.flush()
                }
            }
            BypassStrategy.TCP_COMBINED_NUCLEAR -> {
                // Nuclear strategy: Zero Window Stall + Multiple Fake Ttl segments + Reorder + OOB
                val sniOffset = TlsParser.findSniOffset(data, length)
                
                // 1. Initial Zero-Window Stall to freeze DPI tracking
                try { TtlHelper.setWindowSize(socket, 0) } catch(e: Throwable) {}
                delay(rnd.nextLong(20, 100))
                
                // 2. Send Multiple Fake segments with low TTL to poison DPI reassembly
                repeat(rnd.nextInt(2, 4)) {
                    val chaos = FakePacketHelper.buildTlsChaosPacket()
                    TtlHelper.setTtl(socket, rnd.nextInt(2, 4))
                    output.write(chaos); output.flush()
                    delay(rnd.nextLong(2, 10))
                }
                
                // 3. Send first bit of real data with tiny window
                TtlHelper.setTtl(socket, 64)
                try { TtlHelper.setWindowSize(socket, rnd.nextInt(2, 16)) } catch(e: Throwable) {}
                val split1 = if (sniOffset != -1) sniOffset + 1 else 2
                output.write(data, 0, split1); output.flush()
                delay(config.delay1)
                
                // 4. Send OOB data to further confuse reassembly
                if (rnd.nextBoolean()) try { socket.sendUrgentData(rnd.nextInt(256)) } catch(e: Throwable) {}
                
                // 5. Restore window and send rest of data fragmented
                try { TtlHelper.setWindowSize(socket, 65535) } catch(e: Throwable) {}
                val remaining = length - split1
                if (remaining > 20) {
                    val split2 = remaining / 2
                    output.write(data, split1, split2); output.flush()
                    delay(config.delay2)
                    output.write(data, split1 + split2, remaining - split2); output.flush()
                } else if (remaining > 0) {
                    output.write(data, split1, remaining); output.flush()
                }
            }
            BypassStrategy.TCP_COMBINED_NUCLEAR -> {
                try {
                    ...
                    // 1. Zero-Window Pulse to confuse DPI state
                    socket.receiveBufferSize = 1
                    ...
                    if (offset != -1 && length > offset + 5) {
                        ...
                        val ghost = FakePacketHelper.buildHandshakeCombo(32)
                        val oldTtl = TtlHelper.getSocketTtl(socket)
                        TtlHelper.setTtl(socket, discoveredTtl)
                        ...
                        socket.receiveBufferSize = rnd.nextInt(2, 8)
                        ...
                        socket.receiveBufferSize = originalSize.coerceAtLeast(32768)
                        output.write(data, split2, length - split2); output.flush()
                    } else {
                        ...
                        delay(rnd.nextLong(20, 50))
                        socket.receiveBufferSize = rnd.nextInt(2, 10)
                        output.write(data, s1, length - s1); output.flush()
                        socket.receiveBufferSize = originalSize
                    }
                } catch (e: Throwable) {
                    try { output.write(data, 0, length); output.flush() } catch(e2: Throwable) {}
            ...
            BypassStrategy.TCP_COMBINED_HYBRID -> {
                ...
                val sniOffset = TlsParser.findSniOffset(data, length)
                if (sniOffset != -1) {
                    try { TtlHelper.setWindowSize(socket, 1) } catch(e: Throwable) {}
